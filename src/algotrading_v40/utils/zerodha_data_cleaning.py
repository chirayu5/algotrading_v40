...
def drop_non_standard_indian_trading_hours(
  ...
  utc = pytz.UTC
  start_time_utc = (
    ist.localize(dt.datetime.combine(dt.date(1900, 1, 1), dt.time(9, 15, 59, 999000)))
    .astimezone(utc)
    .time()
  )
  end_time_utc = (
    ist.localize(dt.datetime.combine(dt.date(1900, 1, 1), dt.time(15, 29, 59, 999000)))
    .astimezone(utc)
    .time()
  )
  mask = (df.index.time >= start_time_utc) & (df.index.time <= end_time_utc)  # type: ignore
...
class FixHighLowValuesResult:
  df: pd.DataFrame
  df_original_high_fixed: pd.DataFrame
  df_original_low_fixed: pd.DataFrame

  @property
  def n_high_fixed(self) -> int:
    return len(self.df_original_high_fixed)

  @property
  def n_low_fixed(self) -> int:
    return len(self.df_original_low_fixed)
...
def fix_high_low_values(
  ...
  return FixHighLowValuesResult(
    df=df,
    df_original_high_fixed=df_original.loc[high_needs_fix],
    df_original_low_fixed=df_original.loc[low_needs_fix],
  )
...
def count_bars_per_trading_day(
  ...
  dates_with_less_than_375 = (date_counts < 375).sum()
  ...
  return CountBarsPerTradingDayResult(
    df=result_df,
    dates_with_less_than_375=dates_with_less_than_375,
  )
...
def analyze_numeric_series_quality(s: pd.Series) -> AnalyzeSeriesQualityResult:
  ...
  return AnalyzeSeriesQualityResult(
    ...
    n_bad_values_at_start=n_bad_values_at_start,
    n_bad_values_at_end=n_bad_values_at_end,
  )
